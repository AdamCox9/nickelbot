<?PHP
	...
	/*
		...
		This is an example of a bot that will make orders based off of $_CONFIG array passed in.
		It will set buy orders for every trading pair available for each adapter where balances permit meeting certain criteria.

		TODO
		 - Set base currencies to buy/sell through config.

	*/

	function make_orders( $Adapters = array(), $_CONFIG = array() ) {

		foreach( $Adapters as $Adapter ) {
			echo "*** make_orders: " . get_class( $Adapter ) . " ***\n";
			...
			foreach( $market_summaries as $market_summary ) {
				...
				if( $buy_price < $sell_price ) { //just in case...
					...
					if( $_CONFIG['DIRECTION'] == "SELL" || $_CONFIG['DIRECTION'] == "BOTH" ) {
